...
if "api_key"   not in st.session_state: st.session_state.api_key   = GEMINI_KEY
if "connected" not in st.session_state: st.session_state.connected = bool(GEMINI_KEY)
...
with st.sidebar:
    ...
    st.markdown("<div style='font-size:12px;color:#7A90AB;margin-bottom:6px;font-family:\"DM Sans\",sans-serif'>Gemini API Key</div>", unsafe_allow_html=True)
    key_input = st.text_input("", placeholder="AIzaSy...", type="password", label_visibility="collapsed")

    if st.button("🔑 Activate Gemini AI"):
        if key_input.startswith("AIzaSy") and len(key_input) > 30:
    ...
    if st.session_state.connected:
        ...
        st.markdown("<div style='background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.25);border-radius:6px;padding:8px 12px;font-size:12px;color:#FDE68A;font-family:\"DM Sans\",sans-serif'>🟡 Demo mode — add key above</div>", unsafe_allow_html=True)
    ...
    1. <a href='https://console.cloud.google.com' target='_blank' style='color:#14B8A6'>console.cloud.google.com</a><br>
    2. APIs & Services → Credentials<br>
    3. Create API Key → paste above
    </div>
    """, unsafe_allow_html=True)
...
def call_gemini(user_msg):
    try:
        ...
        model   = genai.GenerativeModel("gemini-2.0-flash", system_instruction=build_prompt())
        history = []
...
with col_chat:
    status_label = "Gemini 2.0 Flash" if st.session_state.connected else "Demo Mode"
    st.markdown(f"""
    <div class="chat-win">
      <div class="chat-head">
